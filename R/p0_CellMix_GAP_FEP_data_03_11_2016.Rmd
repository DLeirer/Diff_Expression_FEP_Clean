...
exprs_into_lumibatch<-exprs_data[rownames(exprs_data)%in%good_fdata_nHSLoc$nuID,]
exprs_into_lumibatch

################ REMOVE DUPLICATES BY SELCTING HIGHEST AVERAGE EXPRESSED PROBE ###############################
#rowmeans
exprsmean<-rowMeans(exprs_into_lumibatch)
#add id
rownames(good_fdata_nHSLoc)<-good_fdata_nHSLoc$nuID
exprsmean_ids<-merge(as.data.frame(exprsmean),good_fdata_nHSLoc[,c("nuID","TargetID")],by="row.names")
...
good_fdata_small<-good_fdata_nHSLoc[exprsmean_ids_unique$Row.names,]
...
pData(eset_bg_log2_rsn_SVA_Good)<-reduced_probs_pheno[,c(11:71,10)]
