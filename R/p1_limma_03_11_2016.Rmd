...
gap <- factor(GX_lumi_object$Phenotype)


#Design Matrix
design = model.matrix(~0+gap)  
...
## Number of sig probes using B-statistic
>>>>>>> a2d4c27737617d68932cf318180116b2fb1f39c2
table(datatoplot$B > 1.5)
...
##Significant probes
datatoplot_sig<-datatoplot[datatoplot$adj.P.Val <= 0.01,]
dim(datatoplot_sig)
...
chrom8=filter(top_de_genes,CHROMOSOME == "8")
arrange(chrom8, PROBE_COORDINATES)[,c(2,10:16)]
head(top_de_genes)
...
ggplot(data = datatoplot, 
      ...
      geom_text(data=filter(datatoplot, logFC >= 0.3 |logFC <= -0.27),check_overlap = TRUE,angle=45,
            aes(CHROMOSOME,logFC,label=TargetID))+
      ggtitle(title)
