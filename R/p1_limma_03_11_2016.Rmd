...
gap_neutro <-as.numeric(eset_bg_log2_rsn_SVA_Good$neutro)
gap_RIN <-as.numeric(eset_bg_log2_rsn_SVA_Good$tech.RIN)
gap_Ampli<-as.factor(as.character(as.numeric(as.factor(eset_bg_log2_rsn_SVA_Good$tech.Date_Dilutionand_Amplification))))

#Design Matrix
design = model.matrix(~0+gap+gap_gender+gap_ethnicity+gap_age+gap_neutro+gap_RIN+gap_Ampli)  
...
# Limma topTabl
top_de_genes <- topTable(contrast.fit, coef=1, number=20000,adjust.method="fdr",p.value=1,confint=TRUE)
dim(top_de_genes)
...
names(datatoplot)
filter(datatoplot[,c(2,30:37)],logFC<=-0.2)
